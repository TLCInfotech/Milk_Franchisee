...
class AddOrEditMilkRouteCollection extends StatefulWidget {
  final AddOrEditMilkRouteCollectionInterface mListener;
  ...
  final route_code;
  final session;
  const AddOrEditMilkRouteCollection(
      {super.key,
  ...
  @override
  State<AddOrEditMilkRouteCollection> createState() => _AddOrEditMilkRouteCollectionState();
}

class _AddOrEditMilkRouteCollectionState extends State<AddOrEditMilkRouteCollection> {
  var kgFactor = 0.9707;
  ...
  getCompanyId() async {
    ...
    print("CompanyIDttttttt=> $companyId");
  }
                      ...
                      Container(
                        ...
                        child: Center(
                          child: widget.route_type=="F"?Text(
                            ...
                            style: page_heading_textStyle,):Text(
  ...
  Widget getAddSearchLayout(double parentHeight, double parentWidth) {
    ...
    return SearchableDropdownWithExistingList(
      ...
      apiUrl:ApiConstants().ledger+"?Group_ID=null&Route_Code=${widget.route_code}&",
      titleIndicator: true,
    ...
    //   SearchableDropdownWithExistingList(
    //   mandatory: true,
    //   txtkey: _partyKey,
    //   focusnext: null,
    //   name: selectedPartyName,
  ...
  callGetRate() async {
    ...
    AppPreferences.getDeviceId().then((deviceId) {
      ...
      String apiUrl = baseurl + ApiConstants().rate + "?Company_ID=$companyId&Date=${DateFormat('yyyy-MM-dd').format(DateTime.parse(widget.date))}&Center_Code=${selectedPartyId}&Milk_Type=${selectedMilkType}&Session=${widget.session}&FAT=${fat.text}&SNF=${snfqty.text}";
      apiRequestHelper.callAPIsForGetAPI(context,apiUrl, model.toJson(), "",
          onSuccess:(data)async{
            // isLoaderShow=false;
            if(mounted){
            ...
            if ( selectedPartyId!=null&& v && q  ) {
              ...
              if (widget.editproduct != null) {
                ...
                item = {
                  ...
                  "Rate": rate.text!=""?rate.text:null,
                  "Amount":amount.text!=""?amount.text:null,
                  // "Qty_In_KG": kgqty.text!=""?double.parse(kgqty.text).toStringAsFixed(2):null,
                  // "Qty_In_Liter": ltrqty.text!=""?double.parse(ltrqty.text).toStringAsFixed(2):null,
                  // "Fat": fat.text!=""?double.parse(fat.text).toStringAsFixed(2):null,
                  // "Lacto": clr.text!=""?double.parse(clr.text).toStringAsFixed(2):null,
                  // "SNF": snfqty.text!=""?double.parse(snfqty.text).toStringAsFixed(2):null,
                  // "Rate": rate.text!=""?double.parse(rate.text).toStringAsFixed(2):null,
                  // "Amount":amount.text!=""?double.parse(amount.text).toStringAsFixed(2):null,
                };
              }

              print(item);
              if (widget.mListener != null) {
                widget.mListener.AddOrEditMilkRouteCollectionDetail(item);
                Navigator.pop(context);
              }
            }
            else {
...
abstract class AddOrEditMilkRouteCollectionInterface {
  AddOrEditMilkRouteCollectionDetail(dynamic item);
}
