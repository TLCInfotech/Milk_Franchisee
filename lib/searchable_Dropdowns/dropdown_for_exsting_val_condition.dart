...
class DropdownExstingValCon extends StatefulWidget{
  final title;
  final ledgerName;
  ...
  final nameField;
  final idField;
  DropdownExstingValCon({required this.title, required this.callback, required this.ledgerName,this.titleIndicator,required this.apiUrl, this.franchisee, this.franchiseeName, this.come,this.suffixicon, this.readOnly,this.focuscontroller,this.txtkey,this.focusnext,this.mandatory, this.nameField, this.idField, this.listExt});




  @override
  State<DropdownExstingValCon> createState() => _SingleLineEditableTextFormFieldState();
}

class _SingleLineEditableTextFormFieldState extends State<DropdownExstingValCon> with  SingleTickerProviderStateMixin {
...
style: item_heading_textStyle,
                  ...
                  onSuggestionSelected: (suggestion) {
                    ...
                    print("<><<<<<<<>");
                    print(suggestion);
                    String basedOnNameN = suggestion['Name']!;
                    String    basedOnIdN = suggestion['ID'].toString();
                    //    onSelectItem(item);
                    // Check if the selected ID is used in the 'Based_On' field of any item in 'items'
                    bool isBasedOnUsed = widget.listExt.any((item) => item['Based_On'] == basedOnIdN);
                    print("klggkgkj  $isBasedOnUsed   ${widget.listExt}");
                    if (isBasedOnUsed==true) {
                      showDialog(
                          context: context,
                          builder: (BuildContext context) {
                            return AlertDialog(
                              backgroundColor: CommonColor.BACKGROUND_COLOR, // Change the background color
                              ...
                              content: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      "${ApplicationLocalizations.of(context).translate("you_can_not")} ${suggestion['Name']} ${ApplicationLocalizations.of(context).translate("as_it_is_already_use")}",
                                      style: page_heading_textStyle.copyWith(fontSize: 16),
                      ...
                      // Perform your action when selection is valid
                      widget.callback(suggestion);
                      print("jrgghgghgh 111");
                    }
